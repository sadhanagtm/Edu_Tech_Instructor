  ...
  const columns = [
    { name: "Name",sortable: true, 
       cell:(row)=>{
        ...
        return(
          <div >
            <Link to={`/course/${row.id}/coursedetail`}>
              {row.name}
            </Link>
          </div>
        )
   ...
   return  <div className="h-8 w-8">
      <img src={image} />
         ...
         { name:"Video",sortable: true, cell:(row) =>{
          let overview=`${import.meta.env.VITE_API_URL}/public/${row.overview}`
     return  <div className="h-8 w-8">
        <video src={overview} />
      </div>
      },
    ...
    { name: "Tags", id:"7", sortable: true, selector: (row) => row.tags },
    {
      ...
      cell: (row) => (
        <div className="flex items-center justify-center gap-4 text-xl">
          <Link to={`/editform/${row.id}`} state={{ courseData: row }}>
